...
def parse_args():
    ...
    parser.add_argument("-b", "--base_path", type=str,
                        ...
                        required=False, 
			default="./data/Metabolomics_Processed_Baseline.tsv")
    parser.add_argument("-e", "--end_path", type=str,
                        help="path to end metabolomics profile in tsv format",
                        required=False, 
			default="./data/Metabolomics_Processed_End.tsv")
    parser.add_argument("-g", "--gem_met_path", type=str,
                        help="path to human gem metabolites list in tsv format",
                        required=False, 
			default="./data/metabolites.tsv")
    ...
    parser.add_argument("-i", "--id_path", type=str,
                        help="path to metabolite identifiers in tsv format",
                        required=False, 
			default="./data/Metabolite_Identifier.tsv")
    parser.add_argument("-l", "--log_path", type=str,
                        help="path to log file",
                        required=False, 
			default="./preprocess_metabolome.log")
    parser.add_argument("-o", "--out_dir", type=str,
                        help="path to output dir",
                        required=False, 
			default="./Preprocessed-Metabolome")
...
def drop_missing_values(base_df, end_df, missing_pct):
    ...
    for diet in diets:
        ...
        print(len(base_df_diet), diet)
        print(len(end_df_diet), diet)
        ...
        print(len(columns_to_keep_baseline))
        print(len(columns_to_keep_end))
                                   
    # Subset columns
    print("End: ", len(columns_to_keep_baseline))
    print("End: ", len(columns_to_keep_end))
...
def impute_missing_values(base_df, end_df, coeff):
    ...
    print(base_df_feature_mins)
    base_df_nan_dict = {}
    end_df_feature_mins = np.min(end_df, axis=0)
    print(end_df_feature_mins)
    end_df_nan_dict = {}
...
def main(args):
    ...
    gem_model['MAM_ID'] = gem_model['REPLACEMENT ID'].str.slice(0,-1)
