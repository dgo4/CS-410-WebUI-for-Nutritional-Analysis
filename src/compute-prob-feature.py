...
def parse_args():
    ...
    parser.add_argument("-b", "--base_path", type=str,
                        ...
                        required=False,
			default="./Preprocessed-Metabolome/gem_overlapped_base_hmdb.tsv")
    parser.add_argument("-e", "--end_path", type=str,
                        help="path to end metabolomics profile in .tsv format",
                        required=False,
			default='./Preprocessed-Metabolome/gem_overlapped_end_hmdb.tsv')
    ...
    parser.add_argument("-l", "--log_path", type=str,
                        ...
                        required=False,
			default='/compute-prob-feature.log')
...
def main(args):
    out_dir = args.out_dir + '/Reaction-Set-' + args.set + '/Prob/'
    ...
    react_df = pd.read_csv(react_set_path, sep='\t', index_col='RXN_ID')
    ...
    sample_nodes = nodes.difference(react_nodes).difference(met_nodes)
    pickle.dump(G, open(out_dir + '/network.pickle', 'wb'))
    
    for treatment in treatments:
        analyze_single_study_per_sample(G, treatment, args.alpha, out_dir)
