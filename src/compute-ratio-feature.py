...
def parse_args():
    ...
    parser.add_argument("-l", "--log_path", type=str,
                        help="log file path",
                        required=False,
			default="compute-ratio-feature.log"),
...
def main(args):
    out_dir = args.out_dir + '/Reaction-Set-' + args.set + '/Ratio/'
    ...
    for treatment in treatments:
        ...
        for idx, rxn in react_set_df.iterrows():
            ...
            for s in rxn['Measured_Substrate']:
                ...
                rc1_col_s = rc1_col_s + study_change_df[met_to_hmdb[s]]
            for p in rxn['Measured_Product']:
                #if(not p in met_to_hmdb):
                print('p', p, idx, rxn['Measured_Product'], rxn)
                rc1_col_p = rc1_col_p + study_change_df[met_to_hmdb[p]]
            rc1_df_dict[rxn['RXN_ID']] = rc1_col_p/rc1_col_s
        ...
        rc1_path = out_dir + '/' + treatment + '.' + control + '.ratio.tsv'
